...
from prompting.take_a_step_back import get_step_back_prediction
...
def evaluate_step_back(data_path, api_key, output_dir="results/step_back", limit=None):
    """Evaluate Take a Step Back prompting technique."""
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            score = get_step_back_prediction(
                row['course_title'],
                row['description'],
    ...
    plot_simple_metrics(
        ...
        results['model_scores'],
        'Take a Step Back',
        f"{output_dir}/step_back_metrics.png"
    )
    
    # Print results summary
    print_simple_summary(metrics, 'Take a Step Back')
...
if __name__ == "__main__":
    ...
    # Run evaluation
    try:
        print("\nStarting Take a Step Back evaluation...")
        print(f"Using data file: {config.DATA_PATH}")
        
        results, metrics = evaluate_step_back(
